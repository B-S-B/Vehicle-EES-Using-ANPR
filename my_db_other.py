...
import sys
sql_password="BSBmysql#1"

# db= mysql.connector.connect(host="localhost", user="root", passwd=sql_password, database="my_test")
# mycourser=db.cursor()
...
class dbhelper:
    def __init__(self):
        ...
        try:
            self.conn=mysql.connector.connect(host="localhost", user="root", database="my_test", passwd=sql_password) # alway check this one from line 13 
    ...
    def register(self,EMP_ID,NAME,PHONE_NO,VECHICLE_NO):
        try:
            self.mycursor.execute("INSERT INTO my_test.anpr VALUES(%s,%s, %s,%s)",(EMP_ID,NAME,PHONE_NO,VECHICLE_NO))
    ...
    def search(self,reg_plate):  ## to read the record from DB 
        ...
        try:
            self.mycursor.execute("SELECT * FROM my_test.anpr WHERE VECHICLE_NO = (%s)",(reg_plate,))
